...
class SasayakiObjectFunction extends SasayakiProvider implements SasayakiInterface
{
     ...
     *
     */
    protected $pattern = "/\@\\\([a-z\_][a-z0-9\_\\\]+[a-z0-9\_])\:\:([a-z\_][a-z0-9\_]+)\s*\=\>\s*\((.*?)\)\;/si";
    ...
    public function search( String $content, Array | AoE\Hairetsu $data ): Void
    {
        $this->content = HTTP\Filter\RegExp::replace( $this->pattern, $content, function( $m ) use( $data ) {
            ...
            if( $m[2] !== "" ) {
                ...
                $result['return'] = Reflection\ReflectionMethod::invoke( $m[1], $m[2], SasayakiParameter::params( $m[2], $result, $data ) );
            } else {
                
                // Invoke function without parameter.
                $result['return'] = Reflection\ReflectionMethod::invoke( $m[1], $m[2] );
            }
