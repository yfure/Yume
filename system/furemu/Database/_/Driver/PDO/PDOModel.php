...
class PDOModel extends Database\Model\ModelProvider implements Database\Model\ModelInterface
{
    ...
    public function insert( Array $column ): Bool
    {
        if( isset( $column[$this->primaryKey] ) === False )
        {
            if( $this->increment )
            {
                ...
                $column = Himei\Arrayable::insert([ 0, $this->primaryKey ], $column, match( $this->incrementValue )
                {
                    
                    // Generate random long numbers.
                    "Number" => Himei\Numberable::length( $this->incrementMax ),
                    
                    // Generate random long string.
                    "String" => Himei\Stringable::random( $this->incrementMax ),
        ...
        {
            $column['created'] = Himei\Application::$object->dateTime->getTimestamp();
        }
    ...
    public function update( Array $column, Array | Bool $where = False ): Bool
    {
        ...
        {
            $column['updated'] = Himei\Application::$object->dateTime->getTimestamp();
        }
        if( $where !== False )
        {
